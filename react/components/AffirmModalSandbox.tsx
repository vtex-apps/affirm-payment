...
class AffirmModal extends Component<AffirmProps> {
  ...
  public componentDidUpdate() {
    ...
    const {
      ...
      settings: { siteHostSuffix },
    } = this.props

    if (
      this.state.hasOrderData &&
      isScriptLoaded &&
      isScriptLoadSucceed &&
      ...
      let hostNameSuffix =
        siteHostSuffix != null && siteHostSuffix != '' ? siteHostSuffix : ''
      ...
      window.affirm.checkout({
        ...
        shipping_amount: Math.round(this.state.shippingValue * 100),
        tax_amount: Math.round(this.state.taxValue * 100),
        total: Math.round(this.state.orderTotal * 100),
      })
  ...
  public queryOrderData = async (g: string) => {
    ...
    if (result.data.orderData != null) {
      ...
      this.setState({
        ...
        shippingLine2:
          result.data.orderData.miniCart.shippingAddress.complement,
...
export default compose(
  withApollo,
  scriptLoader('https://cdn1-sandbox.affirm.com/js/v2/affirm.js')
)(AffirmModal)
